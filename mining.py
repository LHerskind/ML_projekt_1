...
attributeNames = ['MPG', 'Cylinders', 'Displacment', 'Horsepower', 'Weight (lbs)', 'Acceleration (MPH)', 'Model year', 'USA', 'Europe', 'Japan']


def mining(input_data):
    ...
    X = binarize(input_data)
    WriteAprioriFile(X, filename = filename)
    WriteAprioriFile(X, titles=attributeNames, filename='carDataLabels.txt')
